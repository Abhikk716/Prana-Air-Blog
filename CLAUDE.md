...
Optional: `GA4_PROPERTY_ID` (numeric id of the "Prana Air - GA4" property; enables the Google Analytics block on the analytics tab) with Google credentials from `GA4_CREDENTIALS_JSON` (raw or base64 JSON of a service-account key or a gcloud `authorized_user` file — needed on Vercel), `GOOGLE_APPLICATION_CREDENTIALS` (path), or the local `~/.config/gcloud/application_default_credentials.json` (found automatically, so `gcloud auth application-default login` is enough for dev). `BLOB_READ_WRITE_TOKEN` (switches uploads to Vercel Blob), `UPLOAD_DIR` (disk path for uploads on production server, e.g. `/html/uploads`), `NEXT_PUBLIC_SITE_URL` (canonical/OG domain, defaults to pranaair.com).
...
### Admin UI
- `app/admin/dashboard/` — server page does auth + slim aggregation (pulls `content` only for never-scored posts via a `$cond`/`$$REMOVE` projection, scores them, then drops it), hands off to `DashboardClient.js` (tabs: `posts`, `analytics`, `banners`; tab selected via `?tab=`). `shared.js` holds the helpers both tabs use (`LANGUAGE_ROWS`, `decodeEntities`, `PostThumb`, the inline stroke `Icon` set — no icon library).
- `AnalyticsTab.js` — the analytics tab. Time range (7D/30D/90D/month/all/custom), category and language filters scope every card. The language filter reads `viewsByLang[lang]` (lifetime and daily); `promotionClicks` has no per-language counter, so click/CTR figures render as "—" under a language filter and the campaign card stays unscoped. `resolveWindow()` derives the window plus the equal-length period before it; one request to `GET /api/admin/analytics?from=YYYY-MM-DD&to=YYYY-MM-DD` covers both and the split happens client-side, so KPI tiles show period-over-period deltas; `?group=month` returns the whole tracker history as per-post, per-month rows for the all-time banner-click chart. Banner clicks have their own chart (daily with previous period, monthly for "All time") and a "Banners" table that lists global/category banners (`BannerSettings`) **and** post-level banners (`Post.promotion` with any field filled) with placement, status, end date, GA views of the posts each runs on, clicks and CTR; a post's own banner always owns that post's clicks, otherwise attribution falls to category then global banner using only the current config. The filter toolbar is `position: sticky` under the site header on screens wider than 900px (header height measured with a ResizeObserver, stuck state via an IntersectionObserver sentinel). "All time" uses lifetime `Post.analytics` totals for the tiles and the last 30 days for the trend/sparklines. The API returns slim rows (`postId`, server-computed `day` key, counters) with no populate — rows are joined to the dashboard's posts by id; the `day` string is built in the server's timezone to match how `/api/analytics/track` stores midnight, so match by string and never re-derive it client-side. Charts: Recharts for the trend (`ComposedChart`, current area + previous-period line) and publishing cadence; ranked HTML bar lists for category/language (values always printed, no donut). Banner attribution is still "current config only" (see the comment in the memo). Views, users, sessions and engagement come **only from Google Analytics 4**; the CMS tracker contributes banner clicks (`promotionClicks`, which GA cannot see), and the post documents contribute metadata, SEO/readability scores, translations and publish dates. Per-post and per-category views are GA numbers too: the GA route returns `bySlug` rows (views split by edition + engagement seconds, up to 2000 slugs) that the tab joins to posts by slug, so "Top performing posts", "Views by category", campaign views, "Never viewed" and "Needs attention" all use GA views. `Post.analytics.views`/`viewsByLang` are no longer displayed. GA data comes from `GET /api/admin/analytics/ga4?from&to[&prevFrom&prevTo][&lang][&category]` (or `?mode=lifetime[&lang][&category]` for "All time") backed by `lib/ga4.js`: it calls the Analytics Data API `batchRunReports` over plain fetch with a hand-rolled OAuth exchange (service-account JWT or `authorized_user` refresh token, Node `crypto`, no google-auth-library), scopes every report to `/blog/` paths minus WordPress admin/feed URLs, maps the language filter to the site's URL folders (`/blog/<code lower-cased>/`, base English = no folder), turns a category into a `FULL_REGEXP` path filter over the slugs of its posts (looked up in Mongo by the route; Google accepted a ~7 KB pattern, the largest category is ~5 KB), and caches responses in memory for 10 minutes. "All time" is a real lifetime query (start `2015-08-14`, the earliest the API accepts) with a `yearMonth` series for the trend and a second last-30-days range so tiles can show "+N last 30 days"; other ranges get a daily series across the window and the previous period. Every toolbar filter (range, custom dates, language, category) therefore scopes every GA figure. If the property id or credentials are missing the route answers `{configured:false, reason}` and the tab shows a setup card instead of failing. The posts tab has multi-select with a bulk bar backed by `POST /api/admin/posts/bulk` (`{ action: 'delete'|'publish'|'draft', ids }`), thumbnails, and SEO/readability score columns.
